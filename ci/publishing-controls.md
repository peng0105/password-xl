...
Gitea 是唯一源码主仓库。构建始终固定读取主仓库和安卓仓库的 master，并锁定实际提交；Gitee 和 GitHub 是主项目的同步仓库。版本由 Jenkins 的 `构建版本` 参数管理，成功后回写主仓库的 `password-xl-web/package.json`。

## 参数

所有五个任务都有可编辑的字符串参数 `构建版本`。运行开始时更新下一次页面默认值，取本任务所选版本 +1 与当前检出的源码版本 +1 中较大者，例如任务仍为 `1.5.4`、源码已为 `1.5.13` 时，默认值直接更新为 `1.5.14`。其他任务发布后，尚未运行的任务页面可能仍显示旧默认值，可清空输入以使用源码下一版本。手动输入不会被静默修改；可手工改为 `1.6.0` 等稳定的三段数字版本。失败也视为用过该版本，重试可手工填回失败版本，同一版本仍不能绑定不同源码。总入口向四个子任务传递相同版本，子任务不再自行加一。
...
任务配置维护见 [中文参数及编码修复](jenkins/chinese-parameters.md)。
