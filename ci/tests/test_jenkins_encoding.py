...
class JenkinsEncoding(unittest.TestCase):
    ...
    def test_migration_preserves_defaults_and_scm_and_is_repeatable(self):
        ...
        self.assertEqual([p.text for p in config.findall('.//parameterDefinitions/*/name')], ['构建版本', '发布OSS', '发布Kubernets'])
        self.assertEqual([p.text for p in config.findall('.//defaultValue')], ['1.5.2', 'false', 'false'])
